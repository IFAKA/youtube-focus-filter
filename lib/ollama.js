...
RULES:
- FOCUS if directly relevant to what the user wants to learn/do right now
- ALLOW if educational, informative, or generally useful (but not directly on-topic)
- ALLOW tutorials, documentaries, lectures, how-tos even if tangentially related
...
Reply with one word: FOCUS, ALLOW, or BLOCK`;
  }
  ...
  async evaluateVideo(videoMetadata, userGoals) {
    ...
    try {
      ...
      if (upper.includes('FOCUS')) decision = 'FOCUS';
      else if (upper.includes('BLOCK')) decision = 'BLOCK';
